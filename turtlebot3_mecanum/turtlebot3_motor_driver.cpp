...
: baudrate_(BAUDRATE),
  ...
  r_left_wheel_id_(DXL_R_LEFT_ID),
  r_right_wheel_id_(DXL_R_RIGHT_ID)
{
  torque_ = false;
...
bool Turtlebot3MotorDriver::setTorque(bool onoff)
{
  ...
  packetHandler_->write1ByteTxRx(portHandler_, DXL_R_LEFT_ID, ADDR_X_TORQUE_ENABLE, onoff, &dxl_error);
  packetHandler_->write1ByteTxRx(portHandler_, DXL_R_RIGHT_ID, ADDR_X_TORQUE_ENABLE, onoff, &dxl_error);
...
bool Turtlebot3MotorDriver::readEncoder4(int32_t &left_value, int32_t &right_value, int32_t &r_left_value, int32_t &r_right_value)
{
  int dxl_comm_result = COMM_TX_FAIL;              // Communication result
  bool dxl_addparam_result = false;                // addParam result
  bool dxl_getdata_result = false;                 // GetParam result
  ...
  dxl_addparam_result = groupSyncReadEncoder_->addParam(r_left_wheel_id_);
  if (dxl_addparam_result != true)
    return false;

  dxl_addparam_result = groupSyncReadEncoder_->addParam(r_right_wheel_id_);
  if (dxl_addparam_result != true)
    return false;
  ...
  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(r_left_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  if (dxl_getdata_result != true)
    return false;

  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(r_right_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  if (dxl_getdata_result != true)
    return false;
  ...
  left_value  = groupSyncReadEncoder_->getData(left_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  right_value = groupSyncReadEncoder_->getData(right_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  r_left_value  = groupSyncReadEncoder_->getData(r_left_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  r_right_value = groupSyncReadEncoder_->getData(r_right_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);

  groupSyncReadEncoder_->clearParam();
  return true;
}

bool Turtlebot3MotorDriver::writeVelocity4(int64_t left_value, int64_t right_value, int64_t r_left_value, int64_t r_right_value)
{
  bool dxl_addparam_result;
  int8_t dxl_comm_result;
  ...
  uint8_t left_data_byte[4] = {0, };
  uint8_t right_data_byte[4] = {0, };
  uint8_t r_left_data_byte[4] = {0, };
  uint8_t r_right_data_byte[4] = {0, };
  ...
  r_left_data_byte[0] = DXL_LOBYTE(DXL_LOWORD(r_left_value));
  r_left_data_byte[1] = DXL_HIBYTE(DXL_LOWORD(r_left_value));
  r_left_data_byte[2] = DXL_LOBYTE(DXL_HIWORD(r_left_value));
  r_left_data_byte[3] = DXL_HIBYTE(DXL_HIWORD(r_left_value));

  dxl_addparam_result = groupSyncWriteVelocity_->addParam(r_left_wheel_id_, (uint8_t*)&r_left_data_byte);
  if (dxl_addparam_result != true)
    return false;

  r_right_data_byte[0] = DXL_LOBYTE(DXL_LOWORD(r_right_value));
  r_right_data_byte[1] = DXL_HIBYTE(DXL_LOWORD(r_right_value));
  r_right_data_byte[2] = DXL_LOBYTE(DXL_HIWORD(r_right_value));
  r_right_data_byte[3] = DXL_HIBYTE(DXL_HIWORD(r_right_value));

  dxl_addparam_result = groupSyncWriteVelocity_->addParam(r_right_wheel_id_, (uint8_t*)&r_right_data_byte);
  if (dxl_addparam_result != true)
    return false;
...
bool Turtlebot3MotorDriver::controlMotor_4wheel(const float wheel_radius, const float wheel_separation, float* value)
{
  bool dxl_comm_result = false;
  ...
  wheel_velocity_cmd[LEFT]   = lin_vel - (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[RIGHT]  = lin_vel + (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[R_LEFT]   = lin_vel - (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[R_RIGHT]  = lin_vel + (ang_vel * wheel_separation / 2);

  wheel_velocity_cmd[LEFT]  = constrain(wheel_velocity_cmd[LEFT]  * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[RIGHT] = constrain(wheel_velocity_cmd[RIGHT] * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[R_LEFT]  = constrain(wheel_velocity_cmd[R_LEFT]  * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[R_RIGHT] = constrain(wheel_velocity_cmd[R_RIGHT] * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);

  dxl_comm_result = writeVelocity4((int64_t)wheel_velocity_cmd[LEFT], (int64_t)wheel_velocity_cmd[RIGHT], (int64_t)wheel_velocity_cmd[R_LEFT], (int64_t)wheel_velocity_cmd[R_RIGHT]);
  if (dxl_comm_result == false)
    return false;
...
bool Turtlebot3MotorDriver::controlMotor_mecanum(const float wheel_radius, const float wheel_separation_x, const float wheel_separation_y, float* value)
{
  bool dxl_comm_result = false;

  float wheel_velocity_cmd[4];
  float lin_vel_x = value[0];
  float lin_vel_y = -value[1];
  float ang_vel = value[5];
  ...
  wheel_velocity_cmd[R_LEFT]  = lin_vel_x - lin_vel_y - (wheel_separation_x + wheel_separation_y) * ang_vel;
  wheel_velocity_cmd[R_RIGHT]  = lin_vel_x + lin_vel_y + (wheel_separation_x + wheel_separation_y) * ang_vel;

...
#include "turtlebot3_mecanum_config.h"
...
void loop()
{
  ...
  if ((t - tTime[0]) >= (1000 / CONTROL_MOTOR_SPEED_FREQUENCY))
  {
    ...
    {
      motor_driver.controlMotor(WHEEL_RADIUS, WHEEL_SEPARATION_X, WHEEL_SEPARATION_Y, zero_velocity);
    }
    else {
...
void publishSensorStateMsg(void)
{
  ...
  dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.r_left_encoder, sensor_state_msg.r_right_encoder);

  if (dxl_comm_result == true)
    updateMotorInfo(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.r_left_encoder, sensor_state_msg.r_right_encoder);
  else
    return;
...
void updateJointStates(void)
{
  ...
  joint_states_pos[R_LEFT]  = last_rad[R_LEFT];
  joint_states_pos[R_RIGHT] = last_rad[R_RIGHT];

  joint_states_vel[LEFT]  = last_velocity[LEFT];
  joint_states_vel[RIGHT] = last_velocity[RIGHT];
  joint_states_vel[R_LEFT]  = last_velocity[R_LEFT];
  joint_states_vel[R_RIGHT] = last_velocity[R_RIGHT];
...
*******************************************************************************/
void updateMotorInfo(int32_t left_tick, int32_t right_tick, int32_t r_left_tick, int32_t r_right_tick)
{
  ...
  if (init_encoder)
  {
    ...
    last_tick[R_LEFT] = r_left_tick;
    last_tick[R_RIGHT] = r_right_tick;
  ...
  current_tick = r_left_tick;

  last_diff_tick[R_LEFT] = current_tick - last_tick[R_LEFT];
  last_tick[R_LEFT]      = current_tick;
  last_rad[R_LEFT]       += TICK2RAD * (double)last_diff_tick[R_LEFT];

  current_tick = r_right_tick;

  last_diff_tick[R_RIGHT] = current_tick - last_tick[R_RIGHT];
  last_tick[R_RIGHT]      = current_tick;
  last_rad[R_RIGHT]       += TICK2RAD * (double)last_diff_tick[R_RIGHT];
}
...
{
  float* orientation;
  double wheel_l, wheel_r, r_wheel_l, r_wheel_r;      // rotation value of wheel [rad]
  ...
  wheel_l = wheel_r = r_wheel_l = r_wheel_r = 0.0;
  ...
  r_wheel_l = TICK2RAD * (double)last_diff_tick[R_LEFT];
  r_wheel_r = TICK2RAD * (double)last_diff_tick[R_RIGHT];
  ...
  // delta_s     = WHEEL_RADIUS * (wheel_r + wheel_l + r_wheel_l + r_wheel_r) / 2.0;
  // theta = WHEEL_RADIUS * (wheel_r - wheel_l) / WHEEL_SEPARATION;
  orientation = sensors.getOrientation();
  theta       = atan2f(orientation[1] * orientation[2] + orientation[0] * orientation[3],
                       0.5f - orientation[2] * orientation[2] - orientation[3] * orientation[3]);

  delta_x     = WHEEL_RADIUS * (wheel_l + wheel_r + r_wheel_l + r_wheel_r) / 4.0f;
  delta_y     = WHEEL_RADIUS * (wheel_l - wheel_r + r_wheel_l - r_wheel_r) / 4.0f;
  delta_theta = theta - last_theta;

  // compute odometric pose
  odom_pose[0] += (delta_x * cos(odom_pose[2]) - delta_y * sin(odom_pose[2]));
  odom_pose[1] += (delta_x * sin(odom_pose[2]) + delta_y * cos(odom_pose[2]));
  ...
  last_velocity[R_LEFT]  = r_wheel_l / step_time;
  last_velocity[R_RIGHT] = r_wheel_r / step_time;
  last_theta = theta;
...
void driveTest(uint8_t buttons)
{
  ...
  motor_driver.readEncoder(current_tick[LEFT], current_tick[RIGHT], current_tick[R_LEFT], current_tick[R_RIGHT]);

  if (buttons & (1 << 0))
  {
    move[0] = true;
    saved_tick[RIGHT] = current_tick[RIGHT];
    saved_tick[LEFT] = current_tick[LEFT];

    diff_encoder = TEST_DISTANCE / (0.319 / 4096); // (Circumference of Wheel) / (The number of tick per revolution)
    tTime[6] = millis();
  }
  else if (buttons & (1 << 1))
  {
    ...
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (0.319 / 4096);
    tTime[6] = millis();
  }
  ...
  else if (move[1])
  {
    if (abs(saved_tick[RIGHT] - current_tick[RIGHT]) <= diff_encoder)
    {
      goal_velocity_from_button[ANGULAR_Z] = -0.7;
      tTime[6] = millis();
    }
    else
    {
      goal_velocity_from_button[ANGULAR_Z]  = 0.0;
      move[1] = false;
    }
  }
}
...
{
  static char *joint_states_name[] = {(char*)"wheel_left_joint", (char*)"wheel_right_joint", (char*)"r_wheel_left_joint", (char*)"r_wheel_right_joint"};
...
void updateGoalVelocity(void)
{
  ...
  sensors.setLedPattern(goal_velocity[LINEAR_X], goal_velocity[ANGULAR_Z]);
}
...
void sendDebuglog(void)
{
  ...
  DEBUG_SERIAL.println("Encoder(r_left) : " + String(encoder[R_LEFT]));
  DEBUG_SERIAL.println("Encoder(r_right) : " + String(encoder[R_RIGHT]));

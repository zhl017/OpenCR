...
void loop()
{
  ...
  if ((t - tTime[0]) >= (1000 / CONTROL_MOTOR_SPEED_FREQUENCY))
  {
    ...
    {
      motor_driver.controlMotor_mecanum(WHEEL_RADIUS, WHEEL_SEPARATION_X, WHEEL_SEPARATION_Y, zero_velocity);
    }
    else
    {
      motor_driver.controlMotor_mecanum(WHEEL_RADIUS, WHEEL_SEPARATION_X, WHEEL_SEPARATION_Y, goal_velocity);
    }
    tTime[0] = t;
  }
...
void publishSensorStateMsg(void)
{
  ...
  dxl_comm_result = motor_driver.readEncoder4(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.r_left_encoder, sensor_state_msg.r_right_encoder);
...
bool calcOdometry(double diff_time)
{
  ...
  if (step_time == 0)
    return false;
  wheel_left_front = TICK2RAD * (double)last_diff_tick[LEFT];
  wheel_right_front = TICK2RAD * (double)last_diff_tick[RIGHT];
  wheel_left_rear = TICK2RAD * (double)last_diff_tick[R_LEFT];
  wheel_right_rear = TICK2RAD * (double)last_diff_tick[R_RIGHT];
  ...
  last_velocity[LEFT]  = wheel_left_front / step_time;
  last_velocity[RIGHT] = wheel_right_front / step_time;
  last_velocity[R_LEFT]  = wheel_left_rear / step_time;
  last_velocity[R_RIGHT] = wheel_right_rear / step_time;
  last_theta = theta;
...
void driveTest(uint8_t buttons)
{
  ...
  static int32_t saved_tick[4] = {0, 0};
  static double diff_encoder = 0.0;

  int32_t current_tick[4] = {0, 0, 0, 0};

  motor_driver.readEncoder4(current_tick[LEFT], current_tick[RIGHT], current_tick[R_LEFT], current_tick[R_RIGHT]);
...
{
  static char *joint_states_name[] = {(char*)"wheel_left_joint", (char*)"wheel_right_joint", (char*)"wheel_r_left_joint", (char*)"wheel_r_right_joint", "joint1", "joint2", "joint3", "joint4", "gripper"};
...
void sendDebuglog(void)
{
  ...
  motor_driver.readEncoder4(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.r_left_encoder, sensor_state_msg.r_right_encoder);
